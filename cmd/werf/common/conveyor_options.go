...
func GetConveyorOptions(commonCmdData *CmdData, imagesToProcess build.ImagesToProcess) (build.ConveyorOptions, error) {
	...
	if len(commonCmdData.GetPlatform()) > 0 {
		specs, err := platformutil.Parse(commonCmdData.GetPlatform())
		if err != nil {
			return build.ConveyorOptions{}, fmt.Errorf("unable to parse platform specs %v: %w", commonCmdData.GetPlatform(), err)
		}

		specs = platformutil.Dedupe(specs)
		conveyorOptions.TargetPlatforms = platformutil.Format(specs)
	}
